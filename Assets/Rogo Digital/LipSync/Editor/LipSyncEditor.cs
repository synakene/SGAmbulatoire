...
public class LipSyncEditor : Editor {
	...
	private float versionNumber = 1.2f;
	...
	void OnEnable () {
		...
		//Ensure all phonemes are marked as valid
		for (int a = 0; a < 10; a++) {
			lsTarget.phonemes[a].verified = true;
		}
		...
		if (lsTarget.blendSystem != null) {
			if (lsTarget.blendSystem.isReady) {
				...
				if (markerTab == 0) {
					int a = 0;
					foreach (PhonemeShape phoneme in lsTarget.phonemes) {
						this.DrawShapeEditor(lsTarget.blendSystem, blendables, lsTarget.useBones, true, phoneme, phoneme.phoneme.ToString() + " Phoneme", a);
						a++;
					}
				} else if (markerTab == 1) {
					int a = 0;
