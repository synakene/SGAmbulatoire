...
public class LipSyncProjectSettings : Editor {
	...
	void OnDisable () {
		LipSyncProject settings = myTarget;
		...
		for (int a = 0; a < myTarget.emotions.Length; a++) {
			...
			if (EditorGUI.EndChangeCheck()) {
				...
				for (int b = 0; b < myTarget.emotions.Length; b++) {
					if ((myTarget.emotions[a] == myTarget.emotions[b] && a != b) ||
						(myTarget.emotions[b].StartsWith(myTarget.emotions[a] + " (") && myTarget.emotions[b].EndsWith(")"))
					) {
						dupCount++;
					}
				}
				...
				if (dupCount > 0) {
					myTarget.emotions[a] += " (" + dupCount.ToString() + ")";
				}
			}
